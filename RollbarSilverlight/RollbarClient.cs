...
using System.Text;
using RollbarSilverlight.Builders;
using System.Net;
using System.IO;

namespace RollbarSilverlight
{
    /// <summary>
    ...
    public class RollbarClient : IRollbarClient
    {
        ...
        protected void HttpPostAsync(string payload, object userParam)
        {
            ...
            {
                // Send packet data
                using (Stream post = request.EndGetRequestStream(requestResult))
                {
                    post.Write(payloadBytes, 0, payloadBytes.Length);
                    post.Close();
                }
                ...
                request.BeginGetResponse(responseResult =>
                {
                    try
                    {
                        OnRequestCompleted(request.EndGetResponse(responseResult), userParam);
                    }
                    catch (WebException webex)
                    {
                        if (webex.Response == null)
                        {
                            var failMsg = string.Format("Request failed. Status: {0}. Message: {1}",
                                                        webex.Status, webex.Message);
                            OnRequestCompleted(new Result(0, failMsg, userParam));
                        }
                        else
                        {
                            OnRequestCompleted(webex.Response, userParam);
                        }
                    }

                    catch (Exception ex)
                    {
                        //report the send error and move on
